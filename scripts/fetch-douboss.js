/**
 * 抖老板自动化数据抓取脚本 (Playwright)
 * 页面: https://www.doulaoban.com/douyin/order-manage/promote-data
 *
 * 安装:  cd scripts && npm install
 * 首次:  node fetch-douboss.js        (打开浏览器手动登录，登录后按 Ctrl+C)
 * 后续:  node fetch-douboss.js        (全自动)
 *
 * 定时: Windows 计划任务每天运行
 */
...
async function setDateRange(page, startDate, endDate) {
  ...
  try {
    ...
    // 2. 精确点击面板内 .arco-picker-shortcuts 中的"昨天"按钮
    const yesterdayBtn = page.locator('.arco-picker-shortcuts >> text=昨天').first();
