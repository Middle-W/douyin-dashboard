...
javascript:(async()=>{
  ...
  // 先回到第1页
  const firstPageBtn=document.querySelector('.arco-pagination-item:not(.arco-pagination-item-ellipsis):not(.arco-pagination-item-previous):not(.arco-pagination-item-next)');
  if(firstPageBtn&&firstPageBtn.textContent.trim()!=='1'){
    const pageOne=document.querySelectorAll('.arco-pagination-item')[0];
  ...
  while(page<=totalPages){
    ...
    const next=document.querySelector('.arco-pagination-item-next, li.arco-pagination-item-next');
